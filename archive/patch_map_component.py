...
orig_body = """    # ── 1. Load island boundary GeoJSON ───────────────────────────────────────
...
new_body = """    # Normalize selected_islands to a list
    ...
    # Load bounds for all selected islands to construct global bounding box
    all_bounds_dict = {}
    ...
    if all_islands_gdf is not None:
        def island_style(feature):
            is_selected = (
                feature["properties"].get("island_key") in islands_list
            )
            return {
                "fillColor":   "#2ecc71" if is_selected else "#95a5a6",
                "color":       "#27ae60" if is_selected else "#7f8c8d",
...
code = code.replace(orig_body, new_body)
